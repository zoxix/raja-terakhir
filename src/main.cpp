...
void reconnect()
{
  ...
  {
    Serial.print("Mengkoneksi ke MQTT...");
    ...
    else
    {
      ...
      Serial.println(" Mecoba reconnect dalam 5 detik");
      delay(5000);
    }
  }
}
...
{
  String NoNode = "node1";
  Serial.print("Mengkoneski ke ");
  Serial.println(host);
  if (!nodemcuClient.connect(host, httpsPort)) {
    Serial.println("Koneksi gaga;");
    return;
  }
  ...
  Serial.println("Permintaan dikirim");
  while (nodemcuClient.connected()) {
    String line = nodemcuClient.readStringUntil('\n');
    if (line == "\r") {
      Serial.println("Header di terima");
      break;
    }
  }
  String line = nodemcuClient.readStringUntil('\n');
  if (line.startsWith("{\"state\":\"success\"")) {
    Serial.println("esp8266/Arduino CI Berhasil!");
  }
  Serial.println("Mendapat Balasan dan Menutup Koneksi");
